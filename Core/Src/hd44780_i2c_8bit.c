...
LineStruct hd44780_read_bulk_bytes(uint8_t device_add,uint8_t line_no, uint8_t cursor_position, _Bool read_to_end)
{
	...
	line_array.array[20] = bytes_read;
	return(line_array);
}
...
void hd44780_set_line(uint8_t device_add, uint8_t line_no, uint8_t cursor_position, bool clear_line)//sets line to 0 to 3 ie 1 to 4
{
    ...
    if (clear_line == true)
    {
        ...
        {
          blank_line[count] = 0x20;// write space to clear line
          count++;
        }
        hd44780_write_bulk_bytes(device_add, blank_line, count);//send the spaces
...
LineStruct hd44780_read_line_from(uint8_t device_add, uint8_t line_no, uint8_t start_cursor, uint8_t finish_cursor)
{
	uint8_t num_chars;
	uint8_t char_count;
	 LineStruct temp_array;
	num_chars = finish_cursor - start_cursor;//number of bytes to be read along line
	if(num_chars == 0) char_count = 1;// just reading wher the cursor is in CGRAM
	else char_count = num_chars;// count the selected number of bytes from start point in CGRAM
	hd44780_set_line(device_add, line_no, start_cursor,POSITION_ONLY );//set CGRAM to this position in selected line do not clear line
	temp_array = hd44780_read_bulk_bytes(device_add, line_no, start_cursor, num_chars);//array element 20 holds the length of read characters
    temp_array.array[20] = char_count;
	return(temp_array);
}
...
{
	hd44780_set_line(device_add, line_no, cursor_position,POSITION_ONLY );//set CGRAM to this position in selected line
	hd44780_write_bulk_bytes(device_add, chars_to_write.array, char_count);//use the stored length in element 20
}
/**
...
{
	    uint8_t message_length; //length of message minus end null attached when string
		char stripped_message[20]; // the character array without the null message ending
	    message_length =  strlen(message); // get the length of string
	    strncpy(stripped_message,message,message_length);//strip off the null string terminator
        hd44780_write_bulk_bytes(device_add, message,message_length);
}

/**
  * @brief   Writes an ASCII message at a specific line and cursor position
  * @brief   Because custom characters are now using Ascii value 0 problems can occur with a zero custom character on a line
  * @brief   hd44780_write _bulk_bytes o, this accomodates a 0 custom charnly uses an array of char
  * @param  (slave_add) hardware device address of slave
  * @param  (message[]) char string or char array
  * @param  (line_no) 0 to 3 not trapped
  * @param  (cursor_position) 0 to 19 not trapped
  * @param  (clear_line) true (1) or false(0) clears from cursor position to position 19 not trapped
  * @param  message_length length of bytes of string "message[]"
  * @param  stripped_message[20] char array , holds message[] minus the null ending
   * @retval  no return value as yet
...
void hd44780_write_message_at(uint8_t device_add,char message[], uint8_t line, uint8_t cursor, bool clear_line)
{
	uint8_t message_length; //length of message minus end null attached when string
	char stripped_message[20]; // the character array without the null message ending
    hd44780_set_line(device_add,line,cursor,clear_line);// set the destination of characters to be written to LCD
    message_length =  strlen(message); // get the length of string
    strncpy(stripped_message,message,message_length);//strip off the null string terminator
    hd44780_write_bulk_bytes(device_add,stripped_message,message_length);// send the message
}
...
void hd44780_write_character(uint8_t device_add,char character)
{
	...
	hd44780_write_bulk_bytes(device_add, single_array, 1);
}
...
{
    sprintf(Ans, "%.5u",Value);
}

void HD44780_format4_hex(uint8_t Value, char * Ans )
{
    sprintf(Ans, "%.1X",Value);
}

void HD44780_format8_hex(uint8_t Value, char * Ans )
{
    sprintf(Ans, "%.2X",Value);
}

void HD44780_format16_hex(int Value, char *Ans)
{
    sprintf(Ans, "%.4X",Value);
}

void hd44780_format8_dec(uint8_t Value, char * Ans)
{
    sprintf(Ans, "%.3u",Value);
}
...
void HD44780_format_bin(uint16_t Value,uint8_t Bits, char * Ans)
{
   uint8_t BitVal;
   uint8_t loop;
   uint8_t AlphaCount = Bits - 1 ;
   for(loop = 0; loop <= Bits - 1; loop++, AlphaCount--)
   {
        BitVal = Value >> loop;
        if(BitVal & 1)
        {
...
uint8_t hex_str_number(char *string)
{
    unsigned long cumulative_val = 0;
    cumulative_val = strtoul(string,0,16);
